...
\hypersetup{unicode=true,
            pdftitle={BSTA 552: Final Exam Review},
            pdfborder={0 0 0},
            breaklinks=true}
...
\setlength{\droptitle}{-2em}
  ...
  \title{BSTA 552: Final Exam Review}
...
\hypertarget{final-review}{%
\section{Final Review}\label{final-review}}

The final exam is technically cumulative but with an emphasis on Chapter
10 material (C\&B). It is closed note and closed book except for two
sides of \emph{two} 8.5x11 inch ``cheat sheets" of notes.
...
Consistency is when a sequence of random variables (depends on \(n\))
converges to a constant (i.e. \(\theta\)) in probability. We want our
...
Efficiency is when an estimator has the smallest variance. In
\emph{finite samples} that means its variance is equal to the Cramer Rao
Lower Bound of the expectation of the estimator (see below).
\emph{Asymptotic efficiency} is when the estimator converges to a normal
distribution with variance equal to the CRLB for one observation.
